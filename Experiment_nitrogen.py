...
Nitrogen injection assisted in-situ upgrading process
"""
from zml_hyd import *
from kineticReaction import create
import numpy as np
...
model = config.create(mesh, porosity=0.05, pore_modulus=1000e6, denc=2600*3000, dist=1.0,
                      ...
                      s=get_initial_s, perm=get_perm, heat_cond=10.0)
...
def pressure_cracking(): 
    ...
    for cell in model.cells:
        ...
        cell.set_attr(config.cell_keys['pre'], 0.0884 * temp + 10.019 )
    return 
...
def run(): 
    ...
    with open(f'results_5_.txt', 'w') as f:
